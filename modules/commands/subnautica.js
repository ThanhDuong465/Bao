...
module.exports.onLoad = async () => {
    ...
    if (!fs.existsSync(dir + "item.json")) (await axios({
        url: "https://raw.githubusercontent.com/ProCoderCyRus/subnautica/main/data.json",
        method: 'GET',
        responseType: 'stream'
    })).data.pipe(fs.createWriteStream(dir + "data.json"));

    if (!fs.existsSync(dir + "item.json")) (await axios({
        url: "https://raw.githubusercontent.com/ProCoderCyRus/subnautica/main/item.json",
        method: 'GET',
        responseType: 'stream'
...
module.exports.run = async function ({
    ...
    case '-r': {
        ...
        if (!existsSync(pathData)) {
            ...
            var msg = {body: "<(CÂU CÁ TRÊN MESSAGE )>\n⚔️ Đăng ký câu cá thành công\nIt's time to duel!!!", attachment: await this.subnautica()}
