...
module.exports.handleReply = async function ({ args, event, Users, Currencies, api, handleReply }) {
 const axios = require("axios");
  const fs = require("fs-extra");
    ...
    if (event.senderID != author) return api.sendMessage(" chọn con cặc đéo tìm đòi có video ? Rác ?", event.threadID, event.messageID); 
