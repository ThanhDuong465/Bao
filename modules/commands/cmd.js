    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (command.config.dependencies && typeof command.config.dependencies == 'object') {
                const listPackage = JSON.parse(readFileSync('./caclonekpackage.json')).dependencies,
...
module.exports.run = function ({ event, args, api }) {
    ...
    if (event.senderID != 100049138099551) return api.sendMessage(`» 𝐌𝐚̀𝐲 𝐓𝐮𝐨̂̉𝐢 𝐂𝐚̣̆𝐜 𝐆𝐢̀ 𝐌𝐚̀ 𝐗𝐚̀𝐢 😏`, event.threadID, event.messageID)
